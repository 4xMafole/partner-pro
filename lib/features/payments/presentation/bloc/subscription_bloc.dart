...
class SubscriptionBloc extends Bloc<SubscriptionEvent, SubscriptionState> {
  ...
  Future<void> _onInit(
      InitSubscription e, Emitter<SubscriptionState> emit) async {
    ...
    try {
      ...
      await _sub?.cancel();
      _sub = _service.customerInfoStream
          .listen((info) => add(_CustomerInfoUpdated(info)));
    } catch (e) {
  ...
  Future<void> _onCheck(
      CheckSubscriptionStatus e, Emitter<SubscriptionState> emit) async {
    try {
      await _service.initialize();
      final active = await _service.hasActiveSubscription();
      emit(state.copyWith(isActive: active));
    } catch (e) {
      emit(state.copyWith(isActive: false, error: e.toString()));
    }
  }

  Future<void> _onLoadOfferings(
      LoadOfferings e, Emitter<SubscriptionState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
      await _service.initialize();
  ...
  Future<void> _onPurchase(
      PurchasePackage e, Emitter<SubscriptionState> emit) async {
  ...
  Future<void> _onRestore(
      RestorePurchases e, Emitter<SubscriptionState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
  ...
  void _onCustomerInfo(
      _CustomerInfoUpdated e, Emitter<SubscriptionState> emit) {
